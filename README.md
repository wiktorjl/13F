...
The page is deliberately minimal: a single centered list of securities with a plain-text header row (Ticker, Name, the view's metric, Price, Day, YTD, Sector) whose labels sort the list, and no filters, search, charts, or fund metadata. Every row links its ticker and name to Yahoo Finance. On phones (under 640px wide) the same list is laid out as a native-feeling list rather than a squeezed table: the logo line and a sideways-scrolling tab row of the four views stay pinned at the top as a sticky header, the Movers toggles and the sort links each take one 44px-tall row (the sort row scrolls sideways instead of wrapping — `Sort  Ticker  Avg Weight ↓  Name  Sector  Price  Day  YTD`, in the order the values appear in a row, the active link underlined with its arrow and scrolled into view when it would otherwise sit off-screen), and each row takes three lines: the bold ticker with its ▲/▼ direction glyph (an unchanged security shows no glyph) and the bold metric on the right, then the name and sector in taupe, then the price and the day (`today`) and YTD (`YTD`) changes, or three taupe dashes when the security is unpriced. Every tab, toggle, sort link, pager link and row link is at least 44px tall, and the page never scrolls sideways. The header nav has four links:
